...
function gameStart() {
    ...
    // Initialize rain system after camera and player are set up
    initRainSystem();

    // --- Score/HighScore ---
    score = 0;
    highScore = parseInt(localStorage[saveName + '_highScore'] || '0', 10);
    lastScoreDistance = Math.floor(playerVehicle.pos.z);
    // --- End Score/HighScore ---
...
function gameUpdateInternal() {
    ...
    // Only allow manual rain toggle if not in stage 2
    if (keyWasPressed('KeyO') && playerLevel !== 2) {
        toggleRain();
        sound_bump.play(0.5, 2);
    }
}
