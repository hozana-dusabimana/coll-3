...
function drawTrack() {
    ...
    const wetness = getWetnessLevel();
    if (wetness > 0) {
        // Darken and make surface more reflective when wet
        const wetColor = trackColor.clone().lerp(BLACK, wetness * 0.3);
        wetColor.a = lerp(wetness, 0.8, 0.4); // Adjust transparency for wet look
        trackColor = wetColor;
    }

    // ... rest of existing drawTrack code ...
}

function drawCars() {
    // ... existing code ...

    // Apply wet effect to vehicles
    const wetness = getWetnessLevel();
    if (wetness > 0) {
        // Make vehicles look wet
        for (const vehicle of vehicles) {
            if (vehicle.color) {
                const wetColor = vehicle.color.clone().lerp(BLACK, wetness * 0.2);
                wetColor.a = lerp(wetness, 0.9, 0.7); // Adjust transparency for wet look
                vehicle.color = wetColor;
            }
        }
    }

    // ... rest of existing drawCars code ...
}